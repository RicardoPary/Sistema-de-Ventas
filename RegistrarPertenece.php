...
function realizaProceso(){
        var parametros = {
                "codigo_empresa" : $('#CodigoEmpresa').val(),
                "codigo_persona" : $('#CodigoPersona').val()		
        };
        $.ajax({
                data:  parametros,
                url:   'CrearPertenece.php',
                type:  'post',
...
$consulta1=$cn->prepare('SELECT * FROM empresa ');
$consulta1->execute();

$consulta2=$cn->prepare('SELECT * FROM proveedor');
$consulta2->execute();
...
<fieldset>
<legend><strong>Informacion Pertenece</strong></legend>
<table width="776" height="74" border="0" cellpadding="2" cellspacing="0">
 ...
 <tr>
   ...
   <td><select size="1" name="select2" id="CodigoEmpresa">
     <?php 
		
	while($result1=$consulta1->fetch())
	{
	?>
     <option value="<?php echo $result1['codigo_empresa']?>"><?php echo $result1['nombre'];?></option>
     <?php 
	}
	?>
   </select></td>
...
<table width="789"  border="0" id="example">
  ...
  <tbody>
	...
	$sql=$cn->prepare('SELECT * FROM pertenece');	
	$sql->execute();		
