...
function realizaProceso(){
        var parametros = {
                "codigo_lista" : $('#CodigoLista').val(),
                "codigo_producto" : $('#CodigoProducto').val()
			
        };
        $.ajax({
                data:  parametros,
                url:   'CrearTieneProducto.php',
                type:  'post',
...
$consulta1=$cn->prepare('SELECT * FROM producto');
$consulta1->execute();
...
<fieldset>
<legend><strong>Añadir Producto a Seccion</strong></legend>
<table width="776" height="74" border="0" cellpadding="2" cellspacing="0">
 ...
 <tr>
   ...
   <td><select size="1" name="select" id="CodigoCliente">
     <?php 
 ...
 <tr>
     ...
     <td width="180" height="26" scope="row"><label for="label">Codigo Lista:</label></td>
     <td width="246"> <div id="seccion">
       <select name="edo">
         <option value="">Codigo Lista</option>
       </select>
     </div>	</td>
     <td width="29">&nbsp;</td>
 </tr>
 ...
 <tr>
     ...
     <td scope="row"><label for="label3">Codigo Producto:</label></td>
     <td><select size="1" name="Genero" id="CodigoProducto">
     <?php 
		
	while($result1=$consulta1->fetch())
	{
	?>
     <option value="<?php echo $result1['codigo_producto']?>"><?php echo $result1['nombre'];?></option>
     <?php 
	}
	?>
   </select></td>
...
<table width="789"  border="0" id="example">
  ...
  <tbody>
	...
	$sql=$cn->prepare('SELECT * FROM tiene_producto');	
	$sql->execute();		
...
<tr>
  <td><span class="Estilo3"><?php echo $op['codigo_lista'];?></span></td>
