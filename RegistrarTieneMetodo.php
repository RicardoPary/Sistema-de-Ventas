...
function realizaProceso(){
        var parametros = {
                "codigo_envio" : $('#CodigoEnvio').val(),
                "codigo_producto" : $('#CodigoProducto').val(),
				"region" : $('#Region').val()			
        };
        $.ajax({
                data:  parametros,
                url:   'CrearTieneMetodo.php',
                type:  'post',
...
$consulta1=$cn->prepare('SELECT * FROM envio ');
$consulta1->execute();

$consulta2=$cn->prepare('SELECT * FROM producto WHERE codigo_producto NOT IN (SELECT codigo_producto FROM tiene_metodo)');
$consulta2->execute();
...
<fieldset>
<legend><strong>Informacion Tiene Metodo</strong></legend>
<table width="776" height="74" border="0" cellpadding="2" cellspacing="0">
          
 <tr>
   <td scope="row">&nbsp;</td>
   <td height="26" scope="row"><label for="label2">Codigo Envio:</label></td>
   <td><select size="1" name="select2" id="CodigoEnvio">
     <?php 
		
	while($result1=$consulta1->fetch())
	{
	?>
     <option value="<?php echo $result1['codigo_envio']?>"><?php echo $result1['nombre'];?></option>
     <?php 
	}
	?>
   </select></td>
 ...
 <tr>
     <td width="16" scope="row">&nbsp;</td>
     <td width="180" height="26" scope="row"><label for="label">Region:</label></td>
     <td width="246">
	 <select size="1" name="select" id="Region">
...
<table width="789"  border="0" id="example">
  ...
  <tbody>
	...
	$sql=$cn->prepare('SELECT * FROM tiene_metodo');	
	$sql->execute();		
