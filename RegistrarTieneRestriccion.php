...
function realizaProceso(){
        var parametros = {
				...
				"codigo_restriccion" : $('#CodigoRestriccion').val()
                		
        };
        $.ajax({
                data:  parametros,
                url:   'CrearTieneRestriccion.php',
                type:  'post',
...
$consulta1=$cn->prepare('SELECT * FROM restriccion ');
$consulta1->execute();
...
<fieldset>
<legend><strong>Informacion Tiene Restriccion</strong></legend>
<table width="487" height="74" border="0" cellpadding="2" cellspacing="0">
          
 <tr>
   <td scope="row">&nbsp;</td>
   <td height="26" scope="row"><label for="label2">Codigo Restriccion:</label></td>
   <td><select size="1" name="select2" id="CodigoRestriccion">
     <?php 
		
	while($result1=$consulta1->fetch())
	{
	?>
     <option value="<?php echo $result1['codigo_restriccion']?>"><?php echo $result1['nombre'];?></option>
     <?php 
	}
	?>
   </select></td>
...
<div style="width:700px; margin:10px auto;">
  <table width="639"  border="0" id="example">
    <thead>
      <tr >
        <td width="128" height="25"><span class="Estilo15">Codigo Restriccion</span></td>
	...
	$sql=$cn->prepare('SELECT * FROM tiene_restriccion');	
	$sql->execute();		
	
  while($op=$sql->fetch())	 	
   { ?>
      <tr>
        <td><span class="Estilo3"><?php echo $op['codigo_restriccion'];?></span></td>
