...
$consulta=$cn->prepare('SELECT * FROM persona WHERE codigo_persona NOT IN (SELECT codigo_persona FROM proveedor) AND codigo_persona NOT IN (SELECT codigo_persona FROM persona_activa) AND codigo_persona NOT IN (SELECT codigo_persona FROM persona_pasiva)');
$consulta->execute();
?>


<fieldset>
<legend><strong>Informacion Persona Pasiva</strong></legend>
<table width="776" height="70" border="0" cellpadding="2" cellspacing="0">
 ...
 <tr>
     ...
     <td scope="row"><label for="label">Fecha Visita:</label></td>
     <td>
...
<table width="788"  border="0" id="example">
  <thead>
    <tr >
	  ...
	  <td width="128" height="25"><span class="Estilo15">Fecha Visita</span></td>
...
include('Conexion.php');
	$sql=$cn->prepare('SELECT * FROM persona_pasiva');	
	$sql->execute();		
...
<tr>
  ...
  <td><span class="Estilo3"><?php echo $op['fecha_visita'];?></span></td>
